...
const buttonItems = [
  { label: "Generate Proposal Word Document" },
  { label: "Generate PPT" },
  { label: "Estimation Excel" },
...
const Proposal = () => {
  ...
  const {selectedFile1,selectedFile2,selectedFile3} = useStateContext()
...
if (!inputValue.trim() || !selectedFile1 || !selectedFile2 || !selectedFile3) return;
  ...
  formData.append("rfp_file", selectedFile1); // send only RFP for now
  formData.append("understanding_file", selectedFile2);
  formData.append("questionnaire_file", selectedFile3);
  ...
  try {
    const res = await fetch("http://localhost:8000/generate_proposal", {
      method: "POST",
      body: formData,
    });
    ...
    <div className="h-screen bg-white flex  ">
      ...
      >
        <div className="content-header">
          <p className="text-3xl">Proposal Generation | {currentChat}</p>
...
<span className="h-2 w-2 bg-gray-400 rounded-full animate-bounce"></span>
        ...
        <div className=" fixed bottom-10 mt-0  p-4 bg-white">
