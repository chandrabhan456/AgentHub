...
export const Delete = ({ data,closeModal }) => {
   const { setIsDelete } = useStateContext();
  ...
  function handlecancel(){
   ...
   const handleDelete = async (userId) => {
    try {
      const response = await fetch(`http://localhost:8000/users/delete_user/?user_id=${userId}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    ...
    >
      ...
      <button type="submit" className="btn" onClick={() => handleDelete(data)}>Confirm</button>
      </div>
      </div>
